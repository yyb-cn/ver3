    ...
    public function index() {
    	if(trim($_REQUEST['search'])=="do"){
	    	...
	    	for($i = 0 ; $i<=$day; $i++){
	    		...
	    		$list[$i]['day'] = $day_time;
	    		//线上充值金额
	    		$list[$i]['online_pay'] = floatval($GLOBALS['db']->getOne("SELECT sum(money) FROM ".DB_PREFIX."payment_notice where is_paid = 1 and create_date = ".$day_date."  and payment_id not in (SELECT id from ".DB_PREFIX."payment where class_name='Otherpay') "));
	    		//线下充值金额
	    		$list[$i]['below_pay'] = floatval($GLOBALS['db']->getOne("SELECT sum(money) FROM ".DB_PREFIX."payment_notice where is_paid = 1 and create_date = ".$day_date." and payment_id in (SELECT id from ".DB_PREFIX."payment where class_name='Otherpay') "));
