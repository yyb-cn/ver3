...
class IndexAction extends AuthAction{
	...
	{		
		...
		//注册待审核
		$register_count = M("User")->where("login_time = 0 and login_ip='' and is_delete=0 and user_type=0")->count();
		$company_register_count = M("User")->where("login_time = 0 and login_ip='' and is_delete=0 and user_type=1")->count();
	...
	function statistics(){
		...
		//注册奖励冻结资金
		$register_lock_money = floatval($GLOBALS['db']->getOne("SELECT sum(lock_money) FROM ".DB_PREFIX."user_money_log where `type` = 18 "));
