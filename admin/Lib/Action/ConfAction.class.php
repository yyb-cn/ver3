...
class ConfAction extends CommonAction{
	...
	{
		$conf_res = M("Conf")->where("is_effect = 1 and `name` in ('INVITE_REFERRALS','INVITE_REFERRALS_MIN','INVITE_REFERRALS_MAX','INVITE_REFERRALS_RATE','REFERRAL_IP_LIMIT')")->order("group_id asc,sort asc")->findAll();
	...
	public function update_referrals(){
		$conf_res = M("Conf")->where("is_effect = 1 and name in ('INVITE_REFERRALS','INVITE_REFERRALS_MIN','INVITE_REFERRALS_MAX','INVITE_REFERRALS_RATE','REFERRAL_IP_LIMIT')")->findAll();
		foreach($conf_res as $k=>$v)
		{
			conf($v['name'],$_REQUEST[$v['name']]);
			if($v['name']=='URL_MODEL'&&$v['value']!=$_REQUEST[$v['name']])
