   ...
   $msg.=" and create_time <".time(); 
	...
	public function account_detail()
	{
		...
		if ($count > 0) {
			...
			$voList = $model->where($map)->order( "`" . $order . "` " . $sort)->findAll ( );
		       foreach ($voList as $k =>$v){
				  $old_money=$v['money'];
				  $old_int=strpos("$old_money","-");
		          $old_user_money==0;
		    
			if($old_int==0){
			
		   $old_subsr = substr($old_money,1);
			$old_user_money+=$old_money;
            ...
            $voList[$k]['old_user_money']=$old_user_money;		
			   }
