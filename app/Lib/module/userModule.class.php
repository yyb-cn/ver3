	...
	if($_POST['pid_name'])
		...
		if(isset($user_data['referer']) && $user_data['referer']!=""){
			...
			if($user_data['pid'] > 0){
				...
				if(intval(app_conf("REFERRAL_IP_LIMIT"))==0 || (intval(app_conf("REFERRAL_IP_LIMIT")) > 0 && $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."user WHERE register_ip ='".get_client_ip()."' AND pid='".$user_data['pid']."'") > 0)){
		...
		if($res['status'] == 1)
		{
			...
			$user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
	         ...
	         $ecv['begin_time'] = get_gmtime();
		     $ecv['ecv_type_id'] = 42;	
			 $ecv['last_time'] = get_gmtime()+604800;
			 $ecv['end_time'] = get_gmtime()+604800;
