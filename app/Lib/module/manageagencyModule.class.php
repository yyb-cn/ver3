...
class manageagencyModule extends SiteBaseModule
{
	 ...
	 */
	 ...
	 function pingzheng(){
		...
		require APP_ROOT_PATH."app/Lib/page.php";
	 	...
	 	$view_info = $GLOBALS['db']->getOne("SELECT view_info FROM ".DB_PREFIX."deal_agency  where id=".$agency_id);
	 ...
	 function add_pingzheng()
	 {
	 	...
	 	$old_view_infos =  $GLOBALS['db']->getOne("SELECT view_info FROM ".DB_PREFIX."deal_agency  where id=".$agency_id);
	 	...
	 	if($GLOBALS['db']->autoExecute(DB_PREFIX."deal_agency",$data,"UPDATE","id=".$agency_id)){
	 ...
	 function delete_pingzheng()
	 {
	 	...
	 	if($GLOBALS['db']->autoExecute(DB_PREFIX."deal_agency",$resulttt,"UPDATE","id=".$agency_id)){
	 		$result['info']="删除成功！";
	 		$result['status'] = 1;
	 	}
	 	else{
	...
	public function save_ips()
	{
		...
		//判断该实名是否存在
		if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_agency where ips_acct_no = '.$ips_acct_no.' and id<> $id ") > 0 ){
			$data['status'] = 0;
	...
	public function save_idsno()
	{
		...
		/*//判断该实名是否存在
		if(acct_type==0 && $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."deal_agency where ips_mer_code = '.$ips_mer_code.' and id<> $id ") > 0 ){
			$data['status'] = 0;
			$data['info'] = "该实名已被其他用户认证，非本人请联系客服";
			ajax_return($data);
		}*/
