...
//手机绑定
	...
	function bid_calculate(){
		...
		$deal['borrow_amount'] = floatval($_POST['money']);
		$deal['rate'] = floatval($_POST['rate']);
		$deal['repay_time'] = floatval($_POST['repay_time']);
		$deal['repay_time_type'] = intval($_POST['repay_time_type']);
		$deal['loantype'] = intval($_POST['loantype']);
		if($deal['repay_time_type']==0){
			$all_manage_money = $deal['borrow_amount'] * floatval($_POST['user_loan_manage_fee']) * 0.01;
		}
		else{
			$all_manage_money = $deal['borrow_amount'] * floatval($_POST['user_loan_manage_fee']) * 0.01 * $deal['repay_time'];
		}
		
		$deal_rs = deal_repay_money($deal);
		echo number_format($deal_rs['remain_repay_money'] - floatval($_POST['money']) - floatval($all_manage_money),2);
	}
}
?>