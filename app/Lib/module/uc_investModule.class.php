		...
		if($temp_user_load){
			...
			if(($temp_user_load["user_id"] == $user_id && intval($temp_user_load['t_user_id']) == 0 )|| $temp_user_load['t_user_id'] == $user_id){
				...
				foreach($temp_user_load['load'] as $kk=>$vv){
					...
					//预期收益
					$temp_user_load['load'][$kk]['yuqi_money']=format_price($vv['month_repay_money']-$vv['self_money'] - $vv['manage_money']);
					//实际收益
					if($vv['has_repay']==1){
						$temp_user_load['load'][$kk]['real_money']=format_price($vv['month_repay_money']- $vv['self_money']+$vv['impose_money'] - $vv['manage_money']);
		...
		if($temp_user_load){
			...
			if($temp_user_load['user_id'] == $user_id){
				...
				foreach($temp_user_load['load'] as $kk=>$vv){
					...
					//实际收益
					if($vv['has_repay']==1)
						$temp_user_load['load'][$kk]['real_money']=format_price($vv['month_repay_money']-$vv['self_money']+$vv['impose_money']);
				}
				$user_load_ids[] = $temp_user_load;
			}
		}
