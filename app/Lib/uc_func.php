...
// +----------------------------------------------------------------------
	...
	function get_user_point_log($limit,$user_id,$type=-1,$condition){
		...
		$user_id = intval($user_id);
		$count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user_point_log where user_id = ".$user_id." $condition");
		$list = array();
		if($count > 0)
			$list = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."user_point_log where user_id = ".$user_id." $condition order by id limit ".$limit);
		return array("list"=>$list,'count'=>$count);
	}
	...
	function get_invite_list($limit,$user_id,$type=0)
	{
		...
		$sql_count = "select count(DISTINCT u.id) from ".DB_PREFIX."user u LEFT JOIN ".DB_PREFIX."deal_load dl ON dl.user_id=u.id and dl.is_repay=0  where u.pid = ".$user_id." $condition group by u.id";
	...
	function getUcRepayPlan($user_id,$status,$limit,$condition=""){
		...
		if($result['rs_count'] > 0){
			...
			foreach($result['list'] as $k=>$v){
				...
				$result['list'][$k]['shiji_money'] = format_price($v['interest_money']+$v['impose_money']);
