			...
			if ($GLOBALS['db']->affected_rows()){	
				...
				foreach($attr as $k=>$v){
					...
					if ($v["pStatus"] == 'Y'){					
						...
						$detail['true_manage_money'] = $ips_detail["pInFee"];
				...
				if (($has_repay_0 == 0 && $has_repay_1 == 0) || ($has_repay_0 == 0 && $has_repay_1 > 0)){
					...
					$deal_rs_sql = "select sum(true_interest_repay) as total_true_interest_repay," .
							...
							"from ".DB_PREFIX."deal_load_repay where deal_id = ".$ipsdata['deal_id'] . " and repay_id = ".$ipsdata['deal_repay_id'];
					
					$deal_rs = $GLOBALS['db']->getRow($deal_rs_sql);
					$last_Rs = $GLOBALS['db']->getRow("SELECT `status`,`true_repay_time` from ".DB_PREFIX."deal_load_repay where deal_id = ".$ipsdata['deal_id'] . " and repay_id = ".$ipsdata['deal_repay_id']." ORDER BY `true_repay_time` DESC");
					...
					$deal_repay_data['true_interest_repay'] =  $deal_rs['total_true_interest_repay'];
					...
					$GLOBALS['db']->autoExecute(DB_PREFIX."deal_repay",$deal_repay_data,"UPDATE","id = ".$ipsdata['deal_repay_id']);
					
					sys_user_status($GLOBALS['user_info']['id'],false,true);
					syn_deal_status($ipsdata['deal_id']);
					syn_transfer_status(0,$ipsdata['deal_id']);
