	...
	if(app_conf("OPEN_AUTOBID") == 1 && $deal_info['ips_bill_no'] ==""){
		if(($deal_info['deal_status'] == 1 || $data['deal_status']==1) && $deal_info['remain_time'] >0 && $deals_time - $deal_info['start_time'] >=1800 && $deal_info['progress_point'] <=95 && $deal_info['repay_time_type'] == 1){
			...
			$sql = "SELECT usa.user_id,usa.fixed_amount,u.user_name,usa.deal_cates FROM ".DB_PREFIX."user_autobid usa " .
					...
					"WHERE (usa.fixed_amount >=d.min_loan_money or d.min_loan_money = 0) and usa.fixed_amount <= d.borrow_amount*0.2 AND usa.is_effect = 1 ".
...
function make_user_repay_plan($deal,$idx,$repay_day,$true_time,$repay_id,&$total_money){
	static $load_users;
	if(!isset($load_users[$deal['id']])){
		$load_users[$deal['id']] = $GLOBALS['db']->getAll("SELECT * FROM ".DB_PREFIX."deal_load  WHERE deal_id=".$deal['id']." ORDER BY id ASC ");
	}
	
	$loantype = intval($deal['loantype']);
	$LoanModule = LoadLoanModule($loantype);
	
	$load_users = $LoanModule->make_user_repay_plan($deal,$idx,$repay_day,$true_time,$repay_id,$load_users[$deal['id']],$total_money);
...
function sys_user_status($user_id,$is_cache = false,$make_cache=false){
	...
	if($data==false){
		...
		$load_info = $GLOBALS['db']->getRow("SELECT (sum(true_interest_money) - sum(true_manage_money) + sum(impose_money)) as load_earnings,(sum(true_repay_money) - sum(true_manage_money)  + sum(impose_money)) AS load_repay_money FROM ".DB_PREFIX."deal_load_repay  WHERE  ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.") AND has_repay=1 ");
		//已赚利息
		...
		$need_load_info = $GLOBALS['db']->getRow("SELECT sum(dr.repay_money) AS load_repay_money,sum(dr.manage_money) as total_manage_money FROM ".DB_PREFIX."deal_repay dr left join ".DB_PREFIX."deal d on dr.deal_id = d.id WHERE dr.has_repay=0 AND dr.user_id=$user_id and d.ips_bill_no =''");
		//待还本息【债权】
		...
		$load_wait = $GLOBALS['db']->getRow("SELECT sum(self_money) as total_self_money,sum(repay_money) as total_repay_money,sum(interest_money) as load_wait_earnings FROM ".DB_PREFIX."deal_load_repay where ((user_id = ".$user_id." and t_user_id = 0) or t_user_id = ".$user_id.") and has_repay = 0  ");
 ...
 */
 function do_loans($id,$repay_start_time,$type = 0){
	...
	$deal_info['repay_start_date'] = to_date($loan_data['repay_start_time'],"Y-m-d");
	...
	if($GLOBALS['db']->affected_rows() > 0){
		...
		$load_list = $GLOBALS['db']->getAll("SELECT id,user_id,`money`,`is_old_loan`,`rebate_money` FROM ".DB_PREFIX."deal_load where deal_id=".$id." and is_rebate = 0 ");
		foreach($load_list as $lk=>$lv){
			//扣除冻结资金
			if($type == 0){
			...
			if(floatval($lv["rebate_money"])!=0){
				$GLOBALS['db']->query("UPDATE ".DB_PREFIX."deal_load SET is_rebate =1 WHERE id=".$lv['id']." AND is_rebate = 0 AND user_id=".$lv['user_id']);
				if($GLOBALS['db']->affected_rows()){
					modify_account(array("money"=>$lv['rebate_money']),$lv['user_id'],"[<a href='".$deal_info['url']."' target='_blank'>".$deal_info['name']."</a>],返利",24);
				}
			}
		}
...
function FW_DESPWD($pwd){
	...
	$pwd = substr($pwd,strlen(LOGIN_DES_KEY()."浦发"),strlen($pwd));
	$pwd = substr($pwd,0,strlen($pwd) - strlen("财富"));
	return $pwd;
}
