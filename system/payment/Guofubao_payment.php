...
class Guofubao_payment implements payment {
    ...
    public function get_payment_code($payment_notice_id) {
		...
		$order_sn = $payment_notice['order_sn'];
        ...
        */
        //新
        $data_front_url =  SITE_DOMAIN.APP_ROOT.'/guofubao_callback.php?act=notify';
        //$data_front_url="";
        $data_return_url = SITE_DOMAIN.APP_ROOT.'/guofubao_callback.php?act=response';
    ...
    public function get_display_code() {
		...
		if($payment_item)
		{
	        ...
	        $html .=".bk_type_0{background-position:15px -908px; }"; //中国建设银行
	        ...
	        $html .=".bk_type_BOCOM{background-position:15px -155px; }"; //交通银行
	        ...
	        $html .=".bk_type_BOBJ{background-position:15px -697px; }"; //北京银行
			$html .=".bk_type_PAB{background-position:15px -827px; }"; //平安银行
			$html .=".bk_type_BOS{background-position:15px -789px; }"; //上海银行
			$html .=".bk_type_NBCB{background-position:15px -649px; }"; //宁波银行
			$html .=".bk_type_NJCB{background-position:15px -743px; }"; //南京银行
			$html .=".bk_type_TCCB{background-position:15px -869px; }"; //天津银行
	        $html .="</style>";
	        $html .="<script type='text/javascript'>function set_bank(bank_id)";
			$html .="{";
			$html .="$(\"input[name='bank_id']\").val(bank_id);";
			$html .="}</script>";
			$html.= "<div class='blank'></div><h3 class='clearfix tl'><b>国付宝支付</b></h3><div class='blank1'></div><hr />";
